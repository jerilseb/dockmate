...
- **Lifecycle actions** behind a confirmation step — every one of them, from start to prune —
  with an in-row spinner while a job runs, and the daemon's own error text if it refuses.
